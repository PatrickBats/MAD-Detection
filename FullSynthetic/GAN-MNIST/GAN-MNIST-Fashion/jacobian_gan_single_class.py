#!/usr/bin/env python3
"""
Single-class Jacobian analysis for FashionMNIST GAN generators.
Samples 100 latent vectors per generation for a single class (Sneaker, class 7).
"""
...
def main():
    ...
    print(f"\nFashionMNIST GAN Single-Class Analysis")
    ...
    # Save results
    with open(f'jacobian_gan_class{CLASS_LABEL}_results.pkl', 'wb') as f:
        pickle.dump({'results': results_dict, 'stats': stats}, f)
    print(f"Saved to jacobian_gan_class{CLASS_LABEL}_results.pkl")
...
def plot_results(results_dict, stats):
    ...
    plt.suptitle(f'FashionMNIST GAN - Class {CLASS_LABEL} ({CLASS_NAME})', fontsize=14)
    plt.tight_layout()
    plt.savefig(f'jacobian_gan_class{CLASS_LABEL}_spectra.png', dpi=150)
    plt.close()

    print(f"Plot saved to jacobian_gan_class{CLASS_LABEL}_spectra.png")
