...
def process_generation(gen, gpu_id, script_dir):
    ...
    try:
        ...
        gen_eigenvalues = []
        for idx, anchor in enumerate(anchors[:20]):  # Use 20 anchors for speed
...
def main():
    ...
    # Compute spectra using 3 GPUs
    all_eigenvalues = {}
    num_gpus = 3

    # Process generations in batches of 3
    for i in range(0, len(generations), num_gpus):
        batch_gens = generations[i:i+num_gpus]
        print(f"Processing batch: {batch_gens}")

        # Create a pool of workers
        with mp.Pool(processes=len(batch_gens)) as pool:
            # Assign each generation to a GPU
            results = pool.starmap(
                process_generation,
                [(gen, idx % num_gpus, script_dir) for idx, gen in enumerate(batch_gens)]
            )

        # Collect results
        for gen, gen_eigenvalues in results:
