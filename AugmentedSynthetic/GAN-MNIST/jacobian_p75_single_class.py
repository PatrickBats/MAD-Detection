#!/usr/bin/env python3
"""
Single-class Jacobian analysis for p=0.75 constant proportion GAN-MNIST.
...
def load_generator(generation, device='cuda'):
    ...
    checkpoint_path = os.path.join(script_dir, 'data', 'gan_outputs_p75', f'generator_{generation}.pth')
...
def main():
    # Generations 0-8 for p=0.75 experiment
    generations = list(range(9))
    num_gpus = torch.cuda.device_count()

    print(f"\np={PROPORTION} GAN-MNIST Single-Class Jacobian Analysis")
    ...
    # Save results
    script_dir = os.path.dirname(os.path.abspath(__file__))
    save_path = os.path.join(script_dir, f'jacobian_p75_class{CLASS_LABEL}_results.pkl')
...
def plot_results(results_dict, stats, save_dir):
    ...
    save_path = os.path.join(save_dir, f'gan_mnist_p75_class{CLASS_LABEL}_spectrum.pdf')
    ...
    save_path = os.path.join(save_dir, f'gan_mnist_p75_class{CLASS_LABEL}_effective_rank.pdf')
    plt.savefig(save_path, bbox_inches='tight')
    plt.close()
    print(f"Effective rank plot saved to {save_path}")


def print_table(stats):
    print("\n" + "="*60)
    print(f"p={PROPORTION} CLASS {CLASS_LABEL} ({CLASS_NAME}) - STATISTICS")
    print("="*60)
