...
#def load_datasets(iteration,save_dir, w, batch_size, n ):
    
#    tf = transforms.Compose([transforms.ToTensor()])
#    dataset = MNIST("./data", train=True, download=True, transform=tf)
#    Xorg = dataset.data
#    Corg = dataset.targets
#    xini = torch.load( save_dir + f"gen_data_with_w_initial_w{w}" )
#    cini = torch.load(save_dir + f"gen_index_with_w_initial_w{w}")
#    xini = xini[0:n]
#    cini = torch.tensor(cini)
#    cini = cini[0:n]
#    X = torch.cat((Xorg,torch.squeeze(xini))) 
#    C = torch.cat( (Corg,cini))
#    for i in range(1,iteration):
#        x = torch.load( save_dir + f"gen_data_with_w{i}_w{w}" )
#        c = torch.load(save_dir + f"gen_index_with_w{i}_w{w}")
#        c = torch.tensor(c)
#        X = torch.cat((X,torch.squeeze(x))) 
#        C = torch.cat( (C,c))
#    dataset.data = X
#    dataset.targets = C
...
def load_datasets(iteration,save_dir, w, batch_size, n ):
    ...
    if iteration == 1:
        ...
        c = torch.tensor(c)
        X = torch.cat((Xorg,torch.squeeze(x))) 
        C = torch.cat( (Corg,c))
    dataset.data = X
    dataset.targets = C
    print(X.size())
