...
For AugmentedSynthetic experiments (file gen 1 = display Gen 0)
"""
...
def main(args):
    ...
    # AugmentedSynthetic uses 1-indexed files (gen 1 = trained on real data)
    data_dir = args.data_dir
    generations = []
    for i in range(1, 20):  # Check gen 1-19 (1-indexed)
        gen_path = os.path.join(data_dir, f'generator_{i}.pth')
        if os.path.exists(gen_path):
            generations.append(i)

    print(f"Found generations (file): {generations}")
    print(f"Display as Gen: {[g-1 for g in generations]} (0-indexed)")

    # Compute or load results
    results_path = os.path.join(args.output_dir, f'jacobian_class{args.class_label}_viz_results.pkl')
    ...
    if os.path.exists(results_path) and not args.recompute:
        ...
        for gen in generations:
            display_gen = gen - 1  # Convert to 0-indexed for display
            print(f"  Processing Gen {display_gen} (file: generator_{gen}.pth)...")
    ...
    for gen_idx, gen in enumerate(generations):
        ...
        # Add generation label (0-indexed)
        first_ax = axes[gen_idx, 0] if len(generations) > 1 else axes[0]
        first_ax.text(-0.3, 0.5, f'Gen {display_gen}', transform=first_ax.transAxes,
    ...
    for gen in generations:
        display_gen = gen - 1  # 0-indexed
        eff_ranks = [s['effective_rank'] for s in results_dict[gen]]
        print(f"{display_gen:<5} {np.mean(eff_ranks):<12.2f} {np.min(eff_ranks):<12.2f} {np.max(eff_ranks):<12.2f}")
