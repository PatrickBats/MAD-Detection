"""
GAN-MNIST Synthetic Augmentation Loop - Paper Style

Implements the synthetic augmentation loop EXACTLY as described in the paper:
- Gen 1: Train on REAL (60k)
- Gen t (t>=2): Train on REAL (60k) + SYNTHETIC from Gen t-1 ONLY (60k) = 120k

Key difference from StyleGAN approach:
- Synthetic data is ONLY from the immediately preceding generation
- NOT accumulated from all previous generations
- This keeps real:synthetic ratio at 1:1 (50% real always)
"""
...
def load_augmented_dataset_paper_style(generation, save_dir, batch_size, real_dataset):
    """
    Load dataset for given generation using PAPER-STYLE synthetic augmentation loop.

    Paper specification (Section 4.1, DDPM on MNIST):
    "the synthetic dataset D_s^t is only sampled from the previous generation G_{t-1}"

    Gen 1: Real only (60k)
    Gen t (t>=2): Real (60k) + Synthetic from Gen t-1 ONLY (60k) = 120k
    ...
    # For generation >= 2: combine real + ONLY previous generation's synthetic data
    ...
    # Load ONLY the previous generation's synthetic data (not accumulated!)
    prev_gen = generation - 1
    data_path = save_dir + f"gen_data_{prev_gen}.pt"
    labels_path = save_dir + f"gen_labels_{prev_gen}.pt"

    if not os.path.exists(data_path):
        raise FileNotFoundError(f"Previous generation data not found: {data_path}")

    syn_X = torch.load(data_path).float()
    # Convert from [0, 255] to [-1, 1]
    syn_X = (syn_X / 255.0) * 2 - 1
    syn_C = torch.load(labels_path).long()

    print(f"  Loaded synthetic data from Gen {prev_gen}: {syn_X.shape[0]} samples")

    # Combine real + previous gen synthetic only
    combined_X = torch.cat([real_X, syn_X], dim=0)
    combined_C = torch.cat([real_C, syn_C], dim=0)

    total_real = real_X.shape[0]
    total_synthetic = syn_X.shape[0]
    total = combined_X.shape[0]
    real_ratio = 100.0 * total_real / total

    print(f"Generation {generation}: Training on {total_real} real + {total_synthetic} synthetic = {total} total")
...
if __name__ == "__main__":
    print(f"Starting PAPER-STYLE Synthetic Augmentation Loop with {generation_number} generations")
    print(f"Saving outputs to: {save_dir}")
    print()
    print("Data structure (Paper-Style - DDPM MNIST approach):")
    print("  Gen 1: REAL (60k)")
    print("  Gen 2: REAL (60k) + SYNTHETIC_Gen1 (60k) = 120k  [50% real]")
    print("  Gen 3: REAL (60k) + SYNTHETIC_Gen2 (60k) = 120k  [50% real]")
    print("  ...")
    print("  Gen t: REAL (60k) + SYNTHETIC_Gen(t-1) (60k) = 120k  [50% real always]")
    print()
    print("NOTE: Unlike StyleGAN approach, synthetic data is NOT accumulated.")
    print("      Only the immediately preceding generation's data is used.")
    print()
    ...
    for generation in range(1, generation_number + 1):
        ...
        # Load augmented dataset (real + previous gen synthetic ONLY)
        dataloader = load_augmented_dataset_paper_style(generation, save_dir, batch_size, real_dataset)

        # Train GAN
        generator = train_gan(dataloader, generation, device, save_dir)

        # Generate samples for the next generation
        print(f"\nGenerating {N_synthetic} samples for next generation...")
    ...
    print(f"\n{'='*60}")
    print("Paper-Style Synthetic Augmentation Loop experiment complete!")
    print(f"{'='*60}")
